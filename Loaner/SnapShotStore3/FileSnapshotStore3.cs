...
namespace Loaner.SnapShotStore3 {
    ...
    public class FileSnapshotStore3 : SnapshotStore 
    {
        ...
        protected override Task<SelectedSnapshot> LoadAsync (string persistenceId, SnapshotSelectionCriteria criteria) {
            try {
                _loadasync++;
                if (_loadasync % 100000 == 0) _log.Info ("LoadAsync() - count of calls={0}", _loadasync);
        ...
        protected override Task SaveAsync (SnapshotMetadata metadata, object snapshot) {
            try {
                _saveasync++;
                if (_saveasync % 100000 == 0) _log.Info ("SaveAsync() - count of calls={0}", _saveasync);
        ...
        private SelectedSnapshot Load (int streamId, SnapshotMetadata metadata) {
            _load++;
            if (_load % 100000 == 0) _log.Info ("Load() - count of calls={0}", _load);
